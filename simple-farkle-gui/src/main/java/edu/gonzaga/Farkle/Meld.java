...
public class Meld {
    ...
    ArrayList<Integer> melds = new ArrayList<>();
  ...
  /*  public void clearScore(){
    ...
    public void removeFromMeld(int value){
        for(int i=0; i<melds.size(); i++){
            if(melds.get(i) == value){
                melds.remove(i);
            }
        }
    }
    ...
    public void checkMeld(){
        ...
        tempScore = combo.getScore();
    }

    public Integer bankedScore(){
        score += tempScore;
        return score;
    }

    
    public int getMeldValue(int index){
    ...
    public void combineDice() {
        ...
        for (int i = 0; i < melds.size(); i++) {
            int diceValue = melds.get(i); // Use melds ArrayList
    ...
    public void addLeftoverDice(ArrayList<Integer> dice) {
        for (int i = 1; i < diceCount.size(); i++) {
            int count = diceCount.get(i);
            while (count > 0) {
                for (int j = 0; j < dice.size(); j++) {
                    if (dice.get(j) == 0) {
                        dice.set(j, i); // Add a die with value i
                        count--;
                        break;
                    }
                }
     ...
     */
     public boolean isFarkle() {
        ArrayList<Integer> copyOfDice = new ArrayList<>();
        int[] counts = new int[7];
        for (int i = 0; i < copyOfDice.size(); i++) {
            int sideUp = copyOfDice.get(i);
            counts[i]++;
        }
    ...
    public Integer getScore(int round){
        int roundScore = 0;
        roundScore = totalScore.get(round);
        return roundScore;
    }
